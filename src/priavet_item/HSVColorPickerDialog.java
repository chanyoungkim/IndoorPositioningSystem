...
public class HSVColorPickerDialog extends AlertDialog {
	...
	private static class HSVColorWheel extends View {
		...
		private void init() {
			...
			pointerPaint
					.setStrokeWidth((int) (density * POINTER_LINE_WIDTH_DP));
		...
		protected void onDraw(Canvas canvas) {
			if (bitmap != null) {
				...
				selectedPoint.x = rect.left
						+ (int) (-FloatMath.cos(hueInPiInterval) * colorHsv[1]
								* innerCircleRadius + fullCircleRadius);
				selectedPoint.y = rect.top
						+ (int) (-FloatMath.sin(hueInPiInterval) * colorHsv[1]
								* innerCircleRadius + fullCircleRadius);

				canvas.drawLine(selectedPoint.x - pointerLength,
						selectedPoint.y, selectedPoint.x + pointerLength,
						selectedPoint.y, pointerPaint);
				canvas.drawLine(selectedPoint.x, selectedPoint.y
						- pointerLength, selectedPoint.x, selectedPoint.y
						+ pointerLength, pointerPaint);
			}
		}
		...
		protected void onSizeChanged(int w, int h, int oldw, int oldh) {
			...
			rect = new Rect(innerPadding, innerPadding, w - innerPadding, h
					- innerPadding);
			bitmap = Bitmap.createBitmap(w - 2 * innerPadding, h - 2
					* innerPadding, Config.ARGB_8888);
			...
			scaledInnerCircleRadius = scaledFullCircleRadius
					* (1 - FADE_OUT_FRACTION);
			scaledFadeOutSize = scaledFullCircleRadius
					- scaledInnerCircleRadius;
		...
		private void createBitmap() {
			...
			for (int i = 0; i < scaledPixels.length; i++) {
				...
				if (centerDist <= scaledFullCircleRadius) {
					...
					if (centerDist <= scaledInnerCircleRadius) {
						...
						alpha = 255 - (int) ((centerDist - scaledInnerCircleRadius)
								/ scaledFadeOutSize * 255);
					}
			...
			for (x = 0; x < w; x++) {
				...
				for (y = 0; y < h; y++) {
					...
					pixels[x * h + y] = scaledPixels[scaledX * scaledHeight
							+ scaledY];
				}
			}
		...
		public int getColorForPoint(int x, int y, float[] hsv) {
			...
			hsv[0] = (float) (Math.atan2(y, x) / Math.PI * 180f) + 180;
			hsv[1] = Math.max(0f,
					Math.min(1f, (float) (centerDist / innerCircleRadius)));
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				if (listener != null) {
					listener.colorSelected(getColorForPoint((int) event.getX(),
							(int) event.getY(), colorHsv));
				}
				invalidate();
				return true;
			}
	...
	private static class HSVValueSlider extends View {
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				int x = Math.max(0,
						Math.min(bitmap.getWidth() - 1, (int) event.getX()));
